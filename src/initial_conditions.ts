...
import {Atom, X, Y} from './Atom';
import {Diatomic} from './Diatomic';
import {zero_cond} from './helper_funcs'
...
export const energies = (bond: number, activation: number) => { return { BOND: bond, ACTV: activation }; };
...
export const E_table: { [key: string]: {BOND: number, ACTV: number} } = {
    XX: energies(2000, 1000),
    YY: energies(2000, 1000),
    XY: energies(9000, 1000),
    YX: energies(9000, 1000)
}
